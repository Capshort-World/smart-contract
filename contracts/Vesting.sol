...
interface IVesting {
    struct Round {
        uint256 roundId;
        string name; // Name of round, ex: Seed, Private, Public, Advisor,...
    ...
    function setRounds(Round[] calldata _rounds) external;

    function setBeneficiaries(Beneficiary[] calldata _beneficiaries) external;

    event RoundSet(uint256 indexed roundId);
...
contract Vesting is IVesting, AccessControl, ReentrancyGuard {
    ...
    uint256 public constant TIME_PER_PERIOD = 30 days;
    /// @notice allow to add beneficiaries when the round they'll involve hasn't started
    bytes32 public constant ADD_BENEFICIARIES_ROLE =
        keccak256("ADD_BENEFICIARIES_ROLE");
    ...
    EnumerableSet.UintSet private roundIds;
    ...
    EnumerableSet.UintSet private beneficiaryIds;
    ...
    constructor(address _token) {
        ...
        _setupRole(ADD_BENEFICIARIES_ROLE, _msgSender());
    }

    /**
     * @dev Getter for the vesting's name.
     * @param roundId Index of round
     */
    function name(uint256 roundId) public view virtual returns (string memory) {
    ...
    function duration(uint256 roundId) public view virtual returns (uint64) {
        return SafeCast.toUint64(rounds[roundId].period * TIME_PER_PERIOD);
    }

    function cliff(uint256 roundId) public view virtual returns (uint64) {
        return SafeCast.toUint64(rounds[roundId].cliff * TIME_PER_PERIOD);
    }

    /**
     * @dev Number of specified beneficiaries
     */
    function currentNumberBeneficiaries() public view returns (uint256) {
    ...
    function released(
        ...
        return beneficiaries[beneficiaryId].released;
    }
    ...
    function claimable(
        ...
        return _vestingSchedule(beneficiaryId) - released(beneficiaryId);
    }

    /// @dev Becareful of passing with too much rounds can lead to unexpected gas limit problems
    /// @param _rounds The rounds information collection according to tokenomics
    function setRounds(
        Round[] calldata _rounds
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_rounds.length > 0, "Nothing to be setup");

        for (uint8 i = 0; i < _rounds.length; i++) {
            require(
                _rounds[i].startTime > block.timestamp,
                "Can not set the time in past"
            );
            require(
    ...
    ) external onlyRole(ADD_BENEFICIARIES_ROLE) {
        require(_beneficiaries.length > 0, "Nothing to be setup");
     ...
     * @param beneficiaryId Index of beneficiary
     * Emits a {ERC20Released} event.
     */
    function release(uint256 beneficiaryId) external virtual nonReentrant {
        require(
            beneficiaries[beneficiaryId].wallet == _msgSender(),
            "Release: invalid contributor"
        );

        _release(beneficiaryId);
    }

    /**
     * @dev Release the tokens that have already vested.
     * @param beneficiaryId metadata index
     * Emits a {ERC20Released} event.
     */
    function _release(uint256 beneficiaryId) internal virtual {
        Beneficiary storage beneficiary = beneficiaries[beneficiaryId];
        require(
            beneficiary.wallet != address(0),
            "Release: Invalid beneficiary address"
        );

        uint256 releasable = _vestingSchedule(beneficiaryId) -
            released(beneficiaryId);
    ...
    function _vestingSchedule(
        ...
        Beneficiary memory beneficiary = beneficiaries[beneficiaryId];
